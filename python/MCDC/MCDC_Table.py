...
class MCDC_Table:
    ...
    def __repr__(self):
        ...
        l = ['{0}\t{1}\n'.format(self._key_str(i), self._val_str(self.table[i])) for i in self.table.keys()]
    ...
    def is_mcdc_covered(self):
        ...
        def flip2(f, c):
            # type: (int, int) -> int
            # TODO: flip = key_f XOR (1<<c)
            mask = 1 << c
            return f ^ mask

        def flip(f, c):
            # type: (int, int) -> int
            # TODO: flip = key OR (1<<c)
        ...
        for cond in range(self.num_truth_values):
            ...
            print('{0}\t0:{1}\t1:{2}'.format(cond, set(self._key_str(f) for f in ff_keys), set(self._key_str(t) for t in tt_keys)))
